...
pub fn get(name_ string) !Server {
	println(' - get ${args.name} server ${name_}')
	name := texttools.name_fix(name_)
	key := '${args.name}_config_${name}'
	mut kvs := fskvs.new(name: 'config')!
	if kvs.exists(key) {
		data := kvs.get(key)!
	...
	return error("can't find server ${args.name} with name @@{name}")
}
...
// run ${args.name} as docker compose
pub fn (mut server Server) start() ! {
	println(' - start ${args.name}: @@{server.name}')
	mut db := postgresql.get(server.config.postgresql_name)!

	// now create the DB
	db.db_create('${args.name}')!

	// mut z := zinit.new()!
	// processname := '${args.name}_${server.name}'
	// mut p := z.process_new(
	// 	name: processname
	// 	cmd: '
...
pub fn (mut server Server) stop() ! {
	print_backtrace()
	println(' - stop ${args.name}: @@{server.name}')
...
pub fn (mut server Server) check() ! {
	...
	p.check()!
	// TODO: need to do some other checks to ${args.name} e.g. rest calls
}
