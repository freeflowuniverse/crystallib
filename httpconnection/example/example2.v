...
conn.cache.allowable_codes << 301 // defaults are [200, 203, 204, 206, 300, 404, 405, 410, 414, 501]
...
res = conn.send(prefix:"posts", params: {"userId": "1"}) ?
println(res.code)
keys = conn.redis.keys('http:$conn.cache.key*') ?
assert keys.len == 1
